...
${url}`);alert('Challenge link copied! Send it to your friend.');return true}catch{prompt('Copy this challenge link:',url);return false}};
  ...
  // Phase 4 utility tools. Input values are never sent to analytics.
  const unitData={
    length:{'Millimetres':1,'Centimetres':10,'Metres':1000,'Kilometres':1000000,'Inches':25.4,'Feet':304.8,'Yards':914.4,'Miles':1609344},
    weight:{'Grams':1,'Kilograms':1000,'Ounces':28.349523125,'Pounds':453.59237},
    temperature:null
  };
  const unitCat=$('unit-category'),unitFrom=$('unit-from'),unitTo=$('unit-to'),unitBtn=$('unit-btn');
  const unitFill=()=>{if(!unitCat||!unitFrom||!unitTo)return;const cat=unitCat.value;const labels=cat==='temperature'?['Celsius','Fahrenheit','Kelvin']:Object.keys(unitData[cat]);[unitFrom,unitTo].forEach((sel,i)=>{sel.innerHTML=labels.map(x=>`<option value="${x}">${x}</option>`).join('');if(i===1&&labels[1])sel.value=labels[1]})};
  if(unitCat){unitFill();unitCat.addEventListener('change',unitFill)}
  if(unitBtn)unitBtn.addEventListener('click',()=>{const cat=unitCat.value,v=Number($('unit-value').value),from=unitFrom.value,to=unitTo.value,r=$('unit-result');if(!Number.isFinite(v)){r.textContent='Enter a valid number.';return}let out;if(cat==='temperature'){let c=from==='Celsius'?v:from==='Fahrenheit'?(v-32)*5/9:v-273.15;out=to==='Celsius'?c:to==='Fahrenheit'?c*9/5+32:c+273.15}else{const base=v*unitData[cat][from];out=base/unitData[cat][to]}const rounded=Math.abs(out)>=100?Number(out.toFixed(4)):Number(out.toFixed(6));r.textContent=`${v} ${from} = ${rounded} ${to}`;send('funhub_tool_action',{tool:'unit_converter',action:'calculate'})});

  const discountBtn=$('discount-btn');if(discountBtn)discountBtn.addEventListener('click',()=>{const p=Number($('discount-price').value),d=Number($('discount-percent').value),r=$('discount-result');if(!Number.isFinite(p)||!Number.isFinite(d)||p<0||d<0||d>100){r.textContent='Enter a valid price and a discount from 0% to 100%.';return}const save=p*d/100,final=p-save;r.textContent=`You save ₹${save.toFixed(2)}. Sale price: ₹${final.toFixed(2)}.`;send('funhub_tool_action',{tool:'discount_calculator',action:'calculate'})});

  const wordText=$('word-text');if(wordText){const updateWord=()=>{const t=wordText.value;const words=t.trim()?t.trim().split(/\s+/).length:0;const lines=t? t.split(/\n/).filter(x=>x.trim()).length:0;$('word-count').textContent=words;$('char-count').textContent=t.length;$('char-no-space-count').textContent=t.replace(/\s/g,'').length;$('line-count').textContent=lines};wordText.addEventListener('input',updateWord);updateWord()}

  const pickerBtn=$('picker-btn');if(pickerBtn)pickerBtn.addEventListener('click',()=>{const r=$('picker-result'),items=($('picker-options').value||'').split(/\n/).map(x=>x.trim()).filter(Boolean).slice(0,100);if(!items.length){r.textContent='Add at least two options, one per line.';return}const pick=items[Math.floor(Math.random()*items.length)];r.textContent=`✦ ${pick}`;send('funhub_tool_action',{tool:'random_picker',action:'pick'})});
  const pickerClear=$('picker-clear');if(pickerClear)pickerClear.addEventListener('click',()=>{$('picker-options').value='';$('picker-result').textContent='Your pick will appear here.'});
